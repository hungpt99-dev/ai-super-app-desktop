...
export interface IBuiltinAgentTemplate {
  /** Human-readable template name shown in the Agents tab "Create" modal. */
  name: string
...
export interface IBuiltinModuleEntry {
  ...
  /** UI metadata used to generate an agent-template card in the Agents tab. */
  ...
  // ── Writing Helper ───────────────────────────────────────────────────────
  {
    ...
    definition: {
      manifest: {
        ...
        minCoreVersion: '1.0.0',
        maxCoreVersion: '2.x',
        permissions: [Permission.AiGenerate, Permission.UiNotify] satisfies readonly Permission[],
