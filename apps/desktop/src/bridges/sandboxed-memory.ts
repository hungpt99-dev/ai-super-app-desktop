 ...
 * ─────────────────────────────────────────────────
 * • "private"  → stored as `agent:{moduleId}`       — default, always writable
...
function resolveScope(
  ...
  if (scope === undefined || scope === 'private') return `agent:${moduleId}`
  if (scope === 'shared') return SHARED_SCOPE
  // Anything else (e.g. 'task:run-123', or a raw 'agent:...' string from the runtime) passes through.
  return scope
}
