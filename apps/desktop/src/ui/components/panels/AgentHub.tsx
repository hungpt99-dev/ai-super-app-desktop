...
function AgentTypeCard({ agentType, isInstalled, isBuiltIn, onInstall, onRemove }: IAgentTypeCardProps): React.JSX.Element {
  const colorClass = agentType.colorClass
  return (
    <div
      className={`flex flex-col rounded-2xl border bg-[var(--color-surface)] p-5 transition-all ${isInstalled
    ...
    >
      ...
      <p className="mb-1 text-sm font-semibold text-[var(--color-text-primary)]">{agentType.name}</p>
...
/**
 * AgentHub — Agent Type Hub.
 * Everything is an agent. Browse and install downloadable agent types.
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--color-bg)]">
      ...
      <div className="shrink-0 border-b border-[var(--color-border)] bg-[var(--color-surface)] px-8 py-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-[var(--color-text-primary)]">Agent Hub</h1>
            <p className="mt-0.5 text-sm text-[var(--color-text-secondary)]">
              Every capability is an agent type. Install types to add them to your Agents tab.
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          {/* Built-in section */}
          {visibleAgentTypes.some((t) => BUILT_IN_IDS.has(t.id)) && (
            <section>
          ...
          {visibleAgentTypes.some((t) => !BUILT_IN_IDS.has(t.id)) && (
            <section>
              ...
              {visibleAgentTypes.filter((t) => !BUILT_IN_IDS.has(t.id)).length === 0 ? (
          ...
          {visibleAgentTypes.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <p className="text-2xl">🔍</p>
              <p className="mt-3 text-sm text-[var(--color-text-secondary)]">No agent types match &quot;{search}&quot;.</p>
              <button onClick={() => { setSearch('') }} className="mt-3 text-xs text-[var(--color-accent)] hover:underline">Clear filters</button>
            </div>
          )}
