...
const DESKTOP_NAV_ITEMS: { id: INavView; label: string; icon: React.ReactNode }[] = [
  {
    id: 'execution-playground',
    label: 'Playground',
    icon: (
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export function Sidebar({ activeView, onNavigate, unreadCount, onNotifications, onSignIn }: ISidebarProps): React.JSX.Element {
  ...
  return (
    <aside className="flex h-screen w-[220px] shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-surface)]">
      {/* Logo / App identity */}
      <div className="flex shrink-0 items-center gap-3 border-b border-[var(--color-border)] px-5 py-[18px]">
      ...
      <nav className="flex flex-col flex-1 min-h-0 gap-1 p-3 overflow-y-auto">
        ...
        <p className="mb-2 mt-4 px-2 text-[10px] font-semibold uppercase tracking-widest text-[var(--color-text-muted)]">
          Build
        </p>
