...
/** Token estimate used in the result footer (cosmetic). */
const TOKENS_PER_WORD = 1.3

// ─── Text-transformation helpers ───────────────────────────────────────────────

function applyImprove(text: string, tone: Tone): string {
  const tonePhrase: Record<Tone, string> = {
    professional: 'This analysis presents',
    casual:       'So basically,',
    persuasive:   'It is clear that',
    academic:     'The evidence suggests that',
  }
  const sentences = text.trim().split(/(?<=[.!?])\s+/)
  const improved = sentences.map((s) => {
    const cleaned = s.replace(/\bi\b/g, 'I').replace(/  +/g, ' ').trim()
    return cleaned.length > 0 ? cleaned.charAt(0).toUpperCase() + cleaned.slice(1) : cleaned
  })
  return `${tonePhrase[tone]} the following: ${improved.join(' ')}`
}

function applySummarize(text: string): string {
  const sentences = text.trim().split(/(?<=[.!?])\s+/).filter(Boolean)
  const keepCount = Math.max(1, Math.ceil(sentences.length * 0.35))
  const kept = sentences.slice(0, keepCount).join(' ')
  return `In summary: ${kept}${!kept.endsWith('.') ? '.' : ''}`
}

function applyExpand(text: string, tone: Tone): string {
  const elaborations: Record<Tone, string> = {
    professional: 'Furthermore, it is worth noting that this perspective aligns with current best practices and industry standards. From a strategic standpoint, this approach offers several distinct advantages that merit careful consideration.',
    casual:       'And honestly, there\'s a lot more to unpack here. When you think about it more deeply, it touches on some really interesting ideas that are definitely worth exploring further.',
    persuasive:   'This point cannot be overstated. The implications are far-reaching, and the evidence overwhelmingly supports the conclusion that decisive action is both necessary and beneficial.',
    academic:     'This phenomenon warrants further scholarly attention. Empirical research in related fields corroborates these findings, suggesting a systematic pattern that transcends individual case studies.',
  }
  return `${text.trim()}\n\n${elaborations[tone]}`
}

function applyTranslate(text: string, language: string): string {
  return `[${language} translation]\n\n${text.trim()}\n\n(Note: This is a simulated translation. Connect to an AI backend for real translations.)`
}

function applyFixGrammar(text: string): string {
  return text
    .replace(/\bi\b/g, 'I')
    .replace(/\s{2,}/g, ' ')
    .replace(/([a-z])\. ([a-z])/g, (_, a: string, b: string) => `${a}. ${b.toUpperCase()}`)
    .replace(/,([^\s])/g, ', $1')
    .trim()
}

function processText(text: string, action: WritingAction, tone: Tone, targetLanguage: string): string {
  switch (action) {
    case 'improve':     return applyImprove(text, tone)
    case 'summarize':   return applySummarize(text)
    case 'expand':      return applyExpand(text, tone)
    case 'translate':   return applyTranslate(text, targetLanguage)
    case 'fix-grammar': return applyFixGrammar(text)
  }
}
      ...
      <div className="flex h-0 flex-1 overflow-hidden">
        ...
        <div className="flex w-1/2 flex-col border-r border-[var(--color-border)] p-5">
          ...
          <div className="mt-2 flex items-center justify-between">
            <span className="text-xs text-[var(--color-text-muted)]">{String(wordCount)} words</span>
            <button
            ...
            >
              ...
              {isLoading ? 'Processing…' : `${activeAction?.icon ?? ''} ${activeAction?.label ?? ''}`}
            </button>
          </div>
        </div>
        ...
        <div className="flex w-1/2 flex-col p-5">
          ...
          {result !== null ? (
            <div className="flex-1 overflow-y-auto rounded-xl border border-[var(--color-accent-dim)] bg-[var(--color-surface)] p-4">
              <p className="whitespace-pre-wrap text-sm leading-relaxed text-[var(--color-text-primary)]">
                {result.text}
              </p>
              <p className="mt-4 text-xs text-[var(--color-text-muted)]">
                {String(result.tokens)} tokens used
              </p>
            </div>
          ) : isLoading ? (
