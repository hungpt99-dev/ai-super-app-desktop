/**
 * Toast.tsx — web dashboard notification toasts.
 *
 * Reads from useNotificationStore. Mount <WebToastContainer /> once
 * at the root (App.tsx) so it floats above all content.
 */
...
const LEVEL_ICONS: Record<IWebNotification['level'], React.JSX.Element> = {
  info: (
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {n.source ? (
          <p className="mt-1 text-[10px] font-medium text-[var(--color-text-muted)]">{n.source}</p>
        ) : null}
      </div>
      <button
        onClick={onDismiss}
...
/**
 * WebToastContainer — fixed overlay for live toast notifications.
 * Mount once inside the authenticated layout.
 */
export function WebToastContainer(): React.JSX.Element {
    ...
    >
      {toasts.map((n) => (
        <ToastItem
          key={n.id}
          n={n}
          onDismiss={() => { useNotificationStore.getState().dismissToast(n.id) }}
        />
      ))}
    </div>
  )
